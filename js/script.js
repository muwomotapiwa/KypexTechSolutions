...
// Ensure Google Analytics tag is present on every page that loads this script
(function ensureGA(){
  try {
    ...
    if (!window.dataLayer) {
      ...
      // Expose gtag on window for compatibility
      window.gtag = gtag;
      gtag('js', new Date());
...
// iOS: prevent auto-zoom on input focus by temporarily disabling zoom
(function preventIOSZoomOnFocus(){
  try {
    ...
    // Safety: restore on orientation change or navigation
