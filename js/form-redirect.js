...
// Ensure GA tag exists even on pages that only include this file
(function ensureGA(){
  try {
...
// iOS: prevent auto-zoom on input focus for pages that only include this file
(function preventIOSZoomOnFocus(){
  try {
    var ua = navigator.userAgent || '';
    var isIOS = /iP(hone|ad|od)/.test(ua);
    if (!isIOS) return;
    if (window.__iosNoZoomInstalled) return; // guard across bundles
